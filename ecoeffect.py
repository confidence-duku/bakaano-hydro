...
PROJECT_DIR_STRUCTURE = {
    "Urban greening": [
        "input_data", "output_data", "input_data/tasmax", 
        "input_data/tasmin", "input_data/srad"
    ],
    "default": [
        "clim", "soil", "elevation", "models", 
        "output_data", "land_cover", "scratch", "shapes"
    ]
}
...
def create_buttons():
    return {
        "download": pn.widgets.Button(name='Download data for scenario analysis', design=Bootstrap,
                                      button_type='light', disabled=True, icon='caret-right'),
        "preprocess": pn.widgets.Button(name='Preprocess data for scenario analysis', design=Bootstrap,
                                        button_type='light', disabled=True, icon='caret-right'),
        "train": pn.widgets.Button(name='Train_model', design=Bootstrap,
                                   button_type='light', disabled=True, icon='caret-right'),
        "evaluate": pn.widgets.Button(name='Show model performance results', design=Bootstrap,
                                      button_type='light', disabled=True, icon='caret-right', sizing_mode='stretch_width'),
        "visualize": pn.widgets.Button(name='Visualize downloaded data', design=Bootstrap,
                                       button_type='light', disabled=True, icon='caret-right'),
        "simulate": pn.widgets.Button(name='Run scenario', design=Bootstrap,
                                      button_type='light', disabled=True, icon='caret-right', sizing_mode='stretch_width'),
        "heat_simulate": pn.widgets.Button(name='Run scenario analysis', design=Bootstrap,
                                      button_type='primary', disabled=False, icon='caret-right'),
        "visualize_simulation": pn.widgets.Button(name='Show scenario analysis results', design=Bootstrap,
                                                  button_type='light', disabled=True, icon='caret-right', sizing_mode='stretch_width'),
    }
...
def create_text_inputs():
    ...
    return {
        "project_name": pn.widgets.TextInput(name='Project name', placeholder='Enter project name...', align='start', width=250, value=unique_id)
    }

def create_radio_groups():
    return {
        "project_type": pn.widgets.RadioBoxGroup(name='Project_type',
                                                 options=['New project', 'Existing project'], inline=True, align='start', design=Material),
        "nbs_options": pn.widgets.RadioBoxGroup(name='nbs_options',
                                                options=['None', 'Forest landscape restoration', 'Forest protected areas', 
                                                         'Wetland conservation', 'Riparian buffer', 
                                                         'Urban greening'], align='start', inline=False, design=Material),
    }

def create_sliders():
    return {
        "temp": pn.widgets.FloatSlider(name='Change in daily temperature (degC)', start=0, end=4, step=0.5, value=1.5, disabled=True),
        "rsds": pn.widgets.FloatSlider(name='Change in daily solar radiation (%)', start=0, end=30, step=2, value=10, disabled=True),
        "ndvi": pn.widgets.FloatSlider(name='Expansion of urban greening areas (%)', start=-30, end=30, step=2, value=10, disabled=True),
        "peak_rf": pn.widgets.FloatSlider(name='Change in frequency of heavy rainfall days (%)', start=-50, end=50, step=5, value=10, disabled=True),
        "dry_days": pn.widgets.FloatSlider(name='Change in frequency of dry days (%)', start=-50, end=50, step=5, value=10, disabled=True),
    }
...
def update_buttons(event):
    ...
    if nbs_value not in  ['None', 'Urban greening']:
        set_button_state(buttons, {
            ...
            "visualize_simulation": {'disabled': False, 'button_type': 'primary'},  # Only activate after simulation
        })
        set_widget_state(sliders, {
            "temp": {'disabled': False},
        ...
        roi_text['roi'].value = 'Double click on a desired catchment area (blue polygon) to select it as study area.'
        ...
        roi_text['roi'].value = 'Zoom in to a desried city or urban area and draw a polygon around it to select it as study area.'
...
def create_panels(buttons, radio_groups, text_inputs):
    wc1 = pn.Card(pn.WidgetBox(radio_groups['nbs_options'], width=300, css_classes=['bk-no-border']),header='### 1. Select a nature-based solution', collapsed=False, design=Material, width=320)
    wc2 = pn.Card(pn.WidgetBox(buttons['evaluate'], width=320, css_classes=['bk-no-border']), header='### 2. Show model reliability', collapsed=True, design=Material, width=320)
    wc3 = pn.Card(sliders['peak_rf'], sliders['dry_days'], sliders['temp'], sliders['rsds'], sliders['ndvi'], buttons['simulate'], buttons['visualize_simulation'], header='### 4. Assess NbS effectiveness', collapsed=False, design=Material, width=320)  
    wc4 = pn.Card(roi_text['roi'], header='### 3. Select a study area',  collapsed=False, design=Material, width=320)
...
styles = {
    'background-color': '#F8F8F8', 'padding': '10px', 'overflow':'hidden'
}

styles1 = {
    'background-color': '#00A170', 'color':'white', 'font-family': 'Arial, sans-serif'
}

styles2 = {
    'background-color': '#F8F8F8', 'color':'white', 'font-family': 'Arial, sans-serif', 'overflow':'auto'
}
...
"""
urban_green_image = pn.pane.Image('https://static.vecteezy.com/system/resources/thumbnails/044/751/695/small_2x/public-garden-in-the-city-panoramic-view-of-city-park-with-green-trees-grass-and-cityscape-on-background-illustration-cartoon-landscape-with-empty-park-and-town-buildings-on-skyline-vector.jpg', sizing_mode='stretch_width', caption='Photo credit:')

wetland_image = pn.pane.Image('https://www.undp.org/sites/g/files/zskgke326/files/migration/zw/UNDP_ZW_AccLab_Wetlands_3.jpg', sizing_mode='stretch_width', caption='Photo credit: UNDP')
...
intro_image = pn.pane.Image('https://ensia.com/wp-content/uploads/2022/03/Voices_nature-positive_main-920x460.jpg', sizing_mode='stretch_width', caption='Photo credit:  Kelsey Kin')

restoration_image = pn.pane.Image('https://www.iucn.org/sites/default/files/content/images/2021/ppt_background2.png', caption='Photo credit: IUCN stock images', sizing_mode='stretch_width')

protected_areas_image = pn.pane.Image('https://ik.imagekit.io/gn7hrls3k/banner_ZMmPXBKdKH.webp',  caption='Photo credit:', sizing_mode='stretch_width')

riparian_image = pn.pane.Image('images/riparian_image.jpg', caption='Photo credit: Jessica Puglisi/Open Space Institute', sizing_mode='stretch_width')

intro = pn.Column(intro_alert, intro_image)

header = pn.Card(pn.pane.HTML("""
<h1>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;EcoEffect: providing data-driven insights on the effectiveness of nature-based solutions</h1>
""", styles=styles1), sizing_mode='stretch_width', styles=styles1, hide_header=True, min_height=80)

#=====================================================================================================================================================
#code logic and project setup
def create_project_structure(project_name, nbs_type):
    dirs_to_create = PROJECT_DIR_STRUCTURE.get(nbs_type, PROJECT_DIR_STRUCTURE['default'])
...
def simulate_out(event):
    ...
    main_page[0] = pn.Tabs(('Flood risk', pn.Card(m3, hide_header=True, sizing_mode='stretch_width')), ('Soil erosion risk', pn.Card(m4, hide_header=True, sizing_mode='stretch_width')), ('Drought risk', pn.Card(m3, hide_header=True, sizing_mode='stretch_width')), width_policy='fit')
...
layer_configs = {
    'Forest landscape restoration': [
        ('common_data/hydrosheds2.geojson', 'Sub-basins', {'weight': 0.5, 'color': 'blue'}),
        ('common_data/wri_reforestation_glo.tif', 'Reforestation opportunities', 'raster'),
    ],
    ...
    if layers:
        for layer in layers:
            if layer[2] == 'raster':
                ...
                m2.add_geojson(layer[0], layer_name=layer[1], info_mode=None, zoom_to_layer=True, style=layer[2])
...
right_sidebar = pn.Card(intro, max_width=450, hide_header=True, sizing_mode='stretch_height')
main = pn.Row(left_sidebar, main_page,  right_sidebar, styles=styles)
new_template = pn.Column(header, main, styles=styles, scroll=False, sizing_mode='stretch_height').servable()
