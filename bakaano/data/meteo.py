...
class Meteo:
    ...
    def check_missing_dates(self, variables=None, scratch_dir=None):
            ...
            if len(parts) >= 3:
                ...
                if var_part in variables:
                    try:
                        ...
                        print(f"Warning: skipping malformed filename {fname}: {e}")
        ...
        if missing:
            print(f"Warning: {len(missing)} missing dates, for example: {missing[:5]}")
        else:
    ...
    def _download_chelsa_data(self, climate_variable, output_folder):
        ...
        if not any(folder.exists() and any(folder.iterdir()) for folder in [self.tasmax_path, self.tasmin_path, self.tmean_path, self.prep_path]):
            ...
            print(f"Meteorology: NetCDF outputs already exist at {self.tasmax_path}, {self.tasmin_path}, {self.tmean_path}, and {self.prep_path}; skipping download.")
    ...
    def _download_era5_land_data(self):
        ...
        if existing_era5_tifs:
            print("Meteorology: existing ERA5 scratch files detected; checking for missing dates only.")
        else:
            ...
            print("Meteorology: ERA5 bulk download completed; verifying files.")
        ...
        print(f"Meteorology: {len(missing_dates)} missing ERA5 dates detected; downloading missing files.")
            ...
            try:
                ...
                if img:
                    ...
                    for band in band_names:
                        ...
                        print(f"Meteorology: downloaded {band} for {date_str}")
            except Exception as e:
                print(f"Meteorology: failed to download {date_str}: {e}")
    
        print("Meteorology: ERA5 download process completed.")
    ...
    def _download_chirps_prep_data(self):
        ...
        if need_bulk_chirps or need_bulk_era5:
            ...
            print("Meteorology: CHIRPS/ERA5 bulk download completed; checking for missing files.")
        else:
            print("Meteorology: existing CHIRPS/ERA5 scratch files detected; checking for missing dates only.")
        ...
        print(f"Meteorology: missing CHIRPS dates: {len(missing_chirps)}")
        ...
        for date_str in missing_chirps:
            try:
                ...
                if img:
                    ...
                    print(f"Meteorology: downloaded CHIRPS for {date_str}")
            except Exception as e:
                print(f"Meteorology: failed CHIRPS download for {date_str}: {e}")
        ...
        print(f"Meteorology: missing ERA5 dates: {len(missing_era5)}")
        ...
        for date_str in missing_era5:
            try:
                ...
                print(f"Meteorology: failed ERA5 download for {date_str}: {e}")
    
        print("Meteorology: CHIRPS and ERA5 download checks completed.")
    ...
    def get_era5_land_meteo_data(self):
        ...
        if self.local_data is False:
            ...
            if missing_vars:
                ...
                print("Meteorology: ERA5 NetCDF outputs already present or rebuilt from scratch files.")
    ...
    def get_chirps_prep_meteo_data(self):
        ...
        if self.local_data is False:
            ...
            if missing_vars:
                ...
                print("Meteorology: CHIRPS/ERA5 NetCDF outputs already present or rebuilt from scratch files.")
    ...
    def get_chelsa_meteo_data(self):
        ...
        if self.local_data is False:
            ...
            with ThreadPoolExecutor(max_workers=4) as executor:
                ...
                for future in futures:
                    ...
                    try:
                        ...
                        print(f"Meteorology: download completed for {variable}")
                    except Exception as e:
                        print(f"Meteorology: error while downloading {variable}: {e}")
            ...
            try:
                ...
                print("Meteorology: local NetCDF paths validated; proceeding with processing.")
            except ValueError as e:
                print(f"Meteorology configuration error: {e}")
            except FileNotFoundError as e:
                print(f"Meteorology file error: {e}")
            except Exception as e:
                print(f"Meteorology: unexpected error while processing local data: {e}")
