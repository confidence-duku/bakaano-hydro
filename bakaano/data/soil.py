...
class Soil:
    ...
    def get_soil_data(self):
        ...
        if not os.path.exists(soil_check):
                ...
                if response.status_code == 200:
                    with open(local_filename, 'wb') as f:
                        for chunk in response.iter_content(chunk_size=8192):
                            f.write(chunk)
                    print(f"Soil: downloaded source raster to {local_filename}")
                else:
                    print(f"Soil: download failed with HTTP status code {response.status_code}")
            ...
            print(f"Soil: processed soil rasters already exist in {self.working_dir}/soil; skipping download.")
