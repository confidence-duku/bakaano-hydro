...
class FloodMapper:
    ...
    def __init__(
        ...
        self.max_hand_m = float(max_hand_m)
        self.max_stage_height_m = float(max_stage_height_m)
        ...
        self._roughness_warning_emitted = False
        self.last_hand_diagnostics = {}
    ...
    def _validate_configuration(self):
        ...
        if self.max_hand_m <= 0:
            raise ValueError(f"max_hand_m must be > 0. Received: {self.max_hand_m}")
        if self.max_stage_height_m <= 0:
            raise ValueError(
                f"max_stage_height_m must be > 0. Received: {self.max_stage_height_m}"
            )
    ...
    def _ensure_raster_has_finite_values(self, path, description):
        """Validate that a raster contains at least one finite data cell."""
        path_str = self._ensure_raster_readable(path, description)
        with rasterio.open(path_str) as src:
            data = src.read(1, masked=True)
            values = np.asarray(data.data, dtype=np.float32)
            values = np.where(np.ma.getmaskarray(data), np.nan, values)
            finite = np.isfinite(values)
            if not np.any(finite):
                raise ValueError(f"{description} contains no finite data cells: {path_str}")
        return path_str
    ...
    def _estimate_channel_slope(self, dem_window, sub_mask, link_length_window, reach_length_m):
        """Estimate reach slope from stream-link cells, not hillslope gradients."""
        stream_mask = sub_mask & np.isfinite(link_length_window) & (link_length_window > 0)
        stream_elev = np.asarray(dem_window[stream_mask], dtype=np.float64)
        stream_elev = stream_elev[np.isfinite(stream_elev)]
        if stream_elev.size < 2 or reach_length_m <= 0:
            raise ValueError("Insufficient stream-link cells for channel slope estimation.")

        elev_drop = float(np.nanpercentile(stream_elev, 95) - np.nanpercentile(stream_elev, 5))
        if not np.isfinite(elev_drop) or elev_drop <= 0:
            raise ValueError("Stream-link elevations do not define a positive channel slope.")
        return max(elev_drop / float(reach_length_m), 1e-6)

    def _estimate_cross_section_widths(self, wetted_counts, resolution, reach_length_m):
        """Estimate effective 1-D top width from wetted planform area.

        Wetted HAND cells form a 2-D planform area. Dividing by reach length is
        a reach-average width approximation; using cell count times resolution
        directly would treat area as a cross-section width and overstate
        discharge for large or irregular subbasins.
        """
        wetted_area = np.asarray(wetted_counts, dtype=np.float64) * float(resolution) ** 2
        effective_length = max(float(reach_length_m), float(resolution))
        top_width = wetted_area / effective_length
        return np.where(wetted_counts > 0, np.maximum(top_width, float(resolution)), 0.0)

    def _estimate_bankfull_depth(self, drainage_area_km2):
        """Estimate bankfull depth from drainage area."""
        drainage_area_km2 = max(float(drainage_area_km2), 1e-6)
    ...
    def _derive_stage_heights_for_subbasin(self, hand_values, drainage_area_km2):
        ...
        hand = hand[np.isfinite(hand) & (hand >= 0) & (hand <= self.max_hand_m)]
        ...
        # --- LOW FLOW: dense resolution near the effective channel depth.
        # Avoid very shallow artificial levels for large basins; they create
        # near-zero discharges that can destabilize the empirical fit.
        low_start = min(bankfull, max(0.05, 0.2 * bankfull))
        low = np.linspace(low_start, bankfull, 5)
        ...
        # --- EXTREME
        # Do not let a single HAND outlier control the rating curve.
        extreme = min(
            max(float(np.nanquantile(hand, 0.99)), bankfull * 4.0),
            self.max_stage_height_m,
        )
    ...
    def _fit_rating_curve(self, stage_height, discharge):
        ...
        # Monotonicity check: Q = k*(h-h0)^m must be strictly increasing.
        # With k>0 and m>0 this always holds analytically, but floating-point
        # errors near h0 can cause Q to be ~0 for the first few levels and then
        # jump.  Verify on the calibration points themselves.
        q_fit = self._rating_curve_model(h, k, h0, m)
        if not np.all(np.diff(q_fit) >= -1e-6 * float(np.max(q_fit))):
            raise ValueError(
                "Fitted rating curve is non-monotonic over the calibration range. "
                "Rejecting to avoid stage-inversion errors."
            )
    ...
    def get_stage_height(
        ...
        stage_clipped = False
        if stage_height > self.max_stage_height_m:
            stage_height = self.max_stage_height_m
            stage_clipped = True
        return stage_height, {
            ...
            "max_stage_height_m": self.max_stage_height_m,
        }
    ...
    def _filter_outlets_in_study_area(self, subbasin_ids, latlist, lonlist):
        """Return outlets inside or on the study-area boundary.

        Boundary outlets are retained intentionally because reach outlets often
        sit at the clipped basin edge. Treat edge outlets with caution when
        interpreting HAND and inundation depth near the domain boundary.
        """
    ...
    def _write_raster(self, output_path, data):
        ...
        profile.update(dtype=rasterio.float32, count=1, nodata=np.nan, compress=None)
    ...
    def _write_grid_raster(self, output_path, data, dtype=None, nodata=np.nan):
        ...
        arr = np.asarray(data)
        profile = self.analysis_dem_profile.copy()
        profile.update(dtype=(dtype or arr.dtype), count=1, nodata=nodata, compress=None)
        write_arr = arr.copy()
        if nodata is not None and np.isfinite(nodata):
            write_arr = np.where(np.isfinite(write_arr), write_arr, nodata)
        elif nodata is not None and not np.issubdtype(write_arr.dtype, np.floating):
            write_arr = np.where(np.isfinite(write_arr), write_arr, nodata)
        with rasterio.open(output_path, "w", **profile) as dst:
    ...
    def _run_wbt_tool(self, overwrite, output_path, func, *args):
        ...
        if not os.path.exists(output_path):
            messages = []
            try:
                result = func(*args, callback=messages.append)
            except TypeError:
                messages = []
                try:
                    result = func(*args)
            ...
            if result not in (None, 0) and result is not True:
                details = "\n".join(str(message) for message in messages[-20:])
                raise RuntimeError(
                    f"WhiteboxTools reported a non-success status while creating {output_path}: {result}"
                    + (f"\nWhiteboxTools output:\n{details}" if details else "")
                )
        self._ensure_raster_has_finite_values(output_path, "WhiteboxTools output raster")

    def _condition_dem_for_hydrology(self, source_dem, output_dem, overwrite=False):
        """Condition DEM using WhiteboxTools: breach then fill.

        ``breach_depressions_least_cost`` is attractive in theory but has two
        practical failure modes:

        1. **Silent hang / OOM** – on large rasters it exhausts memory or spins
           indefinitely because the least-cost graph is built for the full grid.
        2. **Residual flat areas** – when the algorithm cannot find a low-cost
           breach path within ``dist`` cells it leaves depressions unfilled,
           causing pysheds to produce erroneous flat-area flow directions and
           disconnected stream networks that make the rating curve non-monotonic
           or produce extreme discharge outliers for some subbasins.

        The replacement strategy uses ``breach_depressions`` (Lindsay 2016) which
        is O(N) and always terminates, followed by ``fill_depressions`` to handle
        any residual single-cell pits that breach cannot resolve.  This matches
        the approach recommended in the WBT manual for hydrological conditioning
        of large DEMs.
        """
        ...
        temp_dem_finite_nodata = str(self.scratch_dir / "temp_dem_finite_nodata.tif")
        breached_dem = str(self.scratch_dir / "breached_dem.tif")
        self._write_wbt_safe_dem(source_dem, temp_dem)

        # Step 1: Breach – opens paths through thin barriers efficiently
        # flat_increment adds a tiny gradient across flats so D8 routing is
        # deterministic; fill_pits handles residual single-cell sinks.
        ...
        #     raise RuntimeError(
        #         f"WhiteboxTools breach_depressions returned non-success status: {ret}"
        #     )
        # self._ensure_raster_readable(breached_dem, "Breached DEM")

        # Step 2: Fill – removes any residual shallow depressions not addressable
        # by breaching (e.g. wide flat sinks at domain edges)
        messages = []
        ...
        if ret in (None, 0, True) and Path(output_dem).exists():
            self._ensure_raster_readable(output_dem, "Conditioned DEM")
            return

        first_details = "\n".join(str(message) for message in messages[-20:])

        # Some WhiteboxTools builds return success but do not write output when
        # the input GeoTIFF uses NaN as nodata. Retry with a finite nodata
        # sentinel while preserving the source raster mask in GeoTIFF metadata.
        self._write_wbt_safe_dem(source_dem, temp_dem_finite_nodata, nodata_value=-32768.0)
        messages = []
        ret = wbt.fill_depressions(
            temp_dem_finite_nodata,
            output_dem,
            fix_flats=True,
            flat_increment=0.001,
            callback=messages.append,
        ...
        if ret not in (None, 0, True):
            details = "\n".join(str(message) for message in messages[-20:])
            raise RuntimeError(
                f"WhiteboxTools fill_depressions returned non-success status: {ret}"
                + (f"\nWhiteboxTools output:\n{details}" if details else "")
            )
        if not Path(output_dem).exists():
            retry_details = "\n".join(str(message) for message in messages[-20:])
            warnings.warn(
                "WhiteboxTools fill_depressions reported success but did not create "
                f"the conditioned DEM. Falling back to internal priority-flood fill: {output_dem}"
                + (f"\nFirst attempt output:\n{first_details}" if first_details else "")
                + (f"\nRetry output:\n{retry_details}" if retry_details else ""),
                RuntimeWarning,
            )
            self._condition_dem_priority_flood(source_dem, output_dem)
        self._ensure_raster_readable(output_dem, "Conditioned DEM")

    def _condition_dem_priority_flood(self, source_dem, output_dem, flat_increment=1e-5):
        """Fill DEM depressions with a local priority-flood fallback.

        This is used only when WhiteboxTools returns success without creating an
        output file. It preserves nodata as a finite GeoTIFF nodata value so the
        subsequent WhiteboxTools D8 products can read the conditioned DEM.
        """
        nodata_value = -32768.0

        with rasterio.open(source_dem) as src:
            raw = src.read(1, masked=True)
            dem = np.asarray(raw.filled(np.nan), dtype=np.float32)
            profile = src.profile.copy()
        ...
        valid = np.isfinite(dem)
        if not np.any(valid):
            raise ValueError(f"DEM has no valid cells to condition: {source_dem}")
        ...
        def push(row, col):
            if valid[row, col] and not visited[row, col]:
                ...
                heapq.heappush(heap, (float(filled[row, col]), int(row), int(col)))
        ...
        nodata_neighbors = (
            (valid & np.pad(~valid[1:, :], ((0, 1), (0, 0)), constant_values=False))
            | (valid & np.pad(~valid[:-1, :], ((1, 0), (0, 0)), constant_values=False))
            | (valid & np.pad(~valid[:, 1:], ((0, 0), (0, 1)), constant_values=False))
            | (valid & np.pad(~valid[:, :-1], ((0, 0), (1, 0)), constant_values=False))
        )
        for row, col in np.argwhere(nodata_neighbors):
        ...
        while heap:
            spill_elevation, row, col = heapq.heappop(heap)
            for drow, dcol in neighbor_offsets:
                nrow = row + drow
                ncol = col + dcol
                if nrow < 0 or nrow >= nrows or ncol < 0 or ncol >= ncols:
                    continue
                if visited[nrow, ncol] or not valid[nrow, ncol]:
                    continue
                visited[nrow, ncol] = True
                if filled[nrow, ncol] <= spill_elevation:
                    filled[nrow, ncol] = spill_elevation + float(flat_increment)
                heapq.heappush(heap, (float(filled[nrow, ncol]), int(nrow), int(ncol)))

        output = np.where(valid, filled, nodata_value).astype(np.float32)
        profile.update(dtype=rasterio.float32, count=1, nodata=nodata_value, compress=None)
        with rasterio.open(output_dem, "w", **profile) as dst:
            dst.write(output, 1)

    def _build_d8_downstream(self, fdir_data, resolution, encoding="esri"):
        """Return downstream indices and per-cell step lengths for D8 flow directions."""
        encoding = str(encoding).lower()
        if encoding != "esri":
            raise ValueError(
                "FloodMapper expects ESRI D8 pointer encoding. "
                f"Received encoding={encoding!r}."
            )
        ...
        finite_codes = set(np.unique(fdir_data[np.isfinite(fdir_data)]).astype(np.int64).tolist())
        valid_codes = set(d8_map) | {0}
        invalid_codes = finite_codes - valid_codes
        if invalid_codes:
            preview = ", ".join(str(code) for code in sorted(invalid_codes)[:10])
            raise ValueError(
                "Flow-direction raster is not valid ESRI D8 pointer encoding. "
                f"Unexpected code(s): {preview}"
            )
    ...
    def _derive_channel_products_pysheds(self, filled_dem, fdir_data, facc_data, stream_mask, resolution):
        ...
        down_row, down_col, step_length = self._build_d8_downstream(fdir_data, resolution, encoding="esri")
        ...
        no_stream = np.int64(-2)
        first_stream_idx = np.full(size, -1, dtype=np.int64)
        valid_stream_indices = stream_indices[valid_dem[stream_indices]]
        if valid_stream_indices.size == 0:
            raise ValueError(
                "No valid stream cells were extracted for HAND derivation. "
                "Lower stream_threshold_ratio or inspect flow_accumulation.tif and streams.tif."
            )
        first_stream_idx[valid_stream_indices] = valid_stream_indices

        def resolve_first_stream(start_idx):
            path = []
            seen = set()
            current = int(start_idx)

            while True:
                resolved = first_stream_idx[current]
                if resolved != -1:
                    break
                if current in seen:
                    resolved = no_stream
                ...
                downstream_idx = int(down_flat[current])
                if downstream_idx < 0 or not valid_dem[downstream_idx]:
                    resolved = no_stream
                    break
                current = downstream_idx

            for path_idx in path:
                first_stream_idx[path_idx] = resolved
            return resolved

        for idx in np.flatnonzero(valid_dem):
            if first_stream_idx[idx] != -1:
                continue
            resolve_first_stream(idx)

        hand_flat = np.full(size, np.nan, dtype=np.float32)
        valid_first_stream = first_stream_idx >= 0
        negative_hand_count = 0
        negative_hand_min = np.nan
        if np.any(valid_first_stream):
            raw_hand = (
                dem_flat[valid_first_stream] - dem_flat[first_stream_idx[valid_first_stream]]
            ).astype(np.float32)
            negative_hand = np.isfinite(raw_hand) & (raw_hand < -1e-6)
            negative_hand_count = int(np.sum(negative_hand))
            if negative_hand_count:
                negative_hand_min = float(np.nanmin(raw_hand[negative_hand]))
                warnings.warn(
                    f"Computed {negative_hand_count} negative HAND cells "
                    f"(minimum {negative_hand_min:.3f} m). Values were floored "
                    "to zero, but this can indicate residual DEM conditioning "
                    "issues, disconnected flow paths, or floodplain depressions.",
                    RuntimeWarning,
                )
            hand_flat[valid_first_stream] = np.maximum(raw_hand, 0.0)

        subbasin_flat = np.full(size, np.nan, dtype=np.float32)
        first_stream_link_ids = np.full(size, -1, dtype=np.int32)
        first_stream_link_ids[valid_first_stream] = link_ids[first_stream_idx[valid_first_stream]]
        valid_segment = valid_first_stream & (first_stream_link_ids > 0)
        if np.any(valid_segment):
            subbasin_flat[valid_segment] = first_stream_link_ids[valid_segment].astype(np.float32)
        ...
        for idx in stream_indices:
            ...
            if (
                downstream_idx >= 0
                and stream_flat[downstream_idx]
                and link_ids[downstream_idx] == link_ids[idx]
            ):
        ...
        unresolved_count = int(np.sum(valid_dem & (first_stream_idx == no_stream)))
        self.last_hand_diagnostics = {
            "valid_cells": int(np.sum(valid_dem)),
            "stream_cells": int(valid_stream_indices.size),
            "resolved_cells": int(np.sum(valid_first_stream)),
            "unresolved_cells": unresolved_count,
            "negative_hand_cells": negative_hand_count,
            "negative_hand_min_m": negative_hand_min,
        }
        if unresolved_count:
            warnings.warn(
                f"{unresolved_count} valid DEM cells did not drain to an extracted "
                "stream cell and were left as NaN in HAND/subbasin rasters. "
                "Consider lowering stream_threshold_ratio or checking D8 connectivity.",
                RuntimeWarning,
            )
    ...
    def _estimate_reach_roughness(self, drainage_area_km2, reach_length_m, mean_bed_slope):
        """Estimate Manning's n from geomorphic properties.

        This is a heuristic screening-level roughness estimate, not a calibrated
        hydraulic parameter. Published or regulatory applications should replace
        it with observed or locally calibrated Manning's n values.
        """
        area = max(float(drainage_area_km2), 1e-6)
        length_km = max(float(reach_length_m) / 1000.0, 1e-6)
        slope = float(mean_bed_slope)
        if not np.isfinite(slope) or slope <= 0:
            slope = 1e-6
        slope = float(np.clip(slope, 1e-6, 0.5))
        ...
        clipped_n = float(np.clip(n, 0.025, 0.12))
        if (not np.isfinite(n) or clipped_n != float(n)) and not self._roughness_warning_emitted:
            warnings.warn(
                "Estimated Manning's n was clipped to the valid 0.025-0.12 range. "
                "FloodMapper roughness is a heuristic screening-level estimate; "
                "use calibrated roughness values for defensible hydraulic mapping.",
                RuntimeWarning,
            )
            self._roughness_warning_emitted = True
        return clipped_n

    def _write_wbt_safe_dem(self, src_path, dst_path, nodata_value=np.nan):
        """Write a DEM with nodata cells masked for WhiteboxTools.

        The default writes NaN nodata, which avoids treating sentinel values as
        real elevations. Some WhiteboxTools builds fail to write output from
        NaN-nodata GeoTIFFs; callers can pass a finite ``nodata_value`` for a
        compatibility retry while preserving the raster nodata metadata.
        """
        with rasterio.open(src_path) as src:
            ...
            if np.isfinite(nodata_value):
                data = np.where(np.isfinite(data), data, float(nodata_value))
            profile = src.profile.copy()
            profile.update(dtype=rasterio.float32, nodata=nodata_value, compress=None)
    ...
    def _prepare_hydrology_products(self, analysis_dem, overwrite=False):
        ...
        self._run_wbt_tool(
            overwrite,
            fdr,
            lambda callback=None: wbt.d8_pointer(
                fil,
                fdr,
                esri_pntr=True,
                callback=callback,
        ...
        self._write_grid_raster(facc, facc_data, dtype=rasterio.float32, nodata=-32768.0)
        self._ensure_raster_has_finite_values(facc, "Study-area-masked flow accumulation raster")
        ...
        fdr_data, _, _, _ = self._load_array(fdr)
        streams_data, _, _, _ = self._load_array(streams)
        stream_mask = study_mask & np.isfinite(streams_data) & (streams_data > 0)
        resolution = abs(float(transform.a))
        channel_products = self._derive_channel_products_pysheds(
            filled_data,
            fdr_data,
            facc_data,
            stream_mask,
        ...
        hand_data = channel_products["hand"].astype(np.float32)
        strlnk_length_data = channel_products["stream_link_length"].astype(np.float32)
        subbasin_data = channel_products["subbasins"].astype(np.float32)
        hand_data = np.where(study_mask, hand_data, np.nan).astype(np.float32)
        invalid_hand = np.isfinite(hand_data) & ((hand_data < 0) | (hand_data > self.max_hand_m))
        if np.any(invalid_hand):
            warnings.warn(
                f"Masked {int(np.sum(invalid_hand))} HAND cells outside the valid "
                f"0-{self.max_hand_m:g} m range. Large HAND values usually indicate "
                "DEM nodata/sentinel artifacts, disconnected flow paths, or terrain "
                "far outside the plausible floodplain.",
                RuntimeWarning,
            )
            hand_data = np.where(invalid_hand, np.nan, hand_data).astype(np.float32)
        ...
        self._write_grid_raster(hand, hand_data, dtype=rasterio.float32, nodata=np.nan)
        self._write_grid_raster(strlnk_length, strlnk_length_data, dtype=rasterio.float32, nodata=np.nan)
        self._write_grid_raster(subbasins, subbasin_data, dtype=rasterio.float32, nodata=np.nan)
    ...
    def compute_rating_curves(self, overwrite=False, min_subbasin_cells=10):
        ...
        for subbasin_id in self.subbasin_ids:
            ...
            dem_window = fil_data[row_slice, col_slice]
            ...
            if length <= 0:
                length = float(resolution)

            hand_values = hand_window[sub_mask]
            valid = np.isfinite(hand_values) & (hand_values >= 0) & (hand_values <= self.max_hand_m)

            if not np.any(valid):
                self._record_skip(subbasin_id, f"no valid HAND values within 0-{self.max_hand_m:g} m")
                continue

            hand_values = hand_values[valid].astype(np.float32)
            try:
                mean_bed_slope = self._estimate_channel_slope(
                    dem_window,
                    sub_mask,
                    link_length_window,
                    length,
                ...
                self._record_skip(subbasin_id, str(exc))
                continue
            ...
            # --- cross-section geometry from HAND ---
            # Convert wetted planform area to an effective reach-average top
            # width. This remains approximate but avoids treating 2-D
            # inundated area as a 1-D cross-section width.
            top_width = self._estimate_cross_section_widths(wetted_counts, resolution, length)
            ...
            # Cross-sectional flow area A = T * mean_depth (trapezoidal approx)
            area = top_width * mean_depth
            ...
            # --- hydraulics ---
            # Wetted perimeter P = T + 2*d  (wide rectangular approximation)
            wetted_perimeter = top_width + 2.0 * mean_depth
            ...
            # NOTE: do NOT re-scale by drainage_area_km2 here.  The cross-section
            # geometry (top_width, mean_depth) already scales with the number of
            # inundated cells, which grows with basin size.  Multiplying by
            # area^0.8 a second time creates a spurious power-law amplification
            # that inflates discharge for large basins and suppresses it for
            # small ones, producing non-monotonic or physically implausible
            # rating curves.
            ...
            try:
                ...
                self._record_skip(subbasin_id, f"rating-curve fit failed: {exc}")
                continue
            ...
            rating_curve_dict[subbasin_id] = {
                ...
                "fit_discharge_min_m3s": float(np.min(q)),
                "fit_discharge_max_m3s": float(np.max(q)),
                "drainage_area_km2": float(drainage_area_km2),
                "reach_length_m": float(length),
                "mean_bed_slope": float(mean_bed_slope),
                "slope_method": "stream_link_elevation_drop_p95_p05_over_reach_length",
                "top_width_method": "wetted_planform_area_divided_by_reach_length",
                "bankfull_depth_m": float(bankfull_depth),
            }
    ...
    def _ensure_subbasin_layers_loaded(self):
        ...
        if self.subbasin_data is None or self.hand_data is None:
            ...
            hand_data = np.where(
                np.isfinite(hand_data) & (hand_data >= 0) & (hand_data <= self.max_hand_m),
                hand_data,
                np.nan,
            ).astype(np.float32)
            self.subbasin_data = subbasin_data
    ...
    def _validate_predicted_streamflow_dates(self, predicted_streamflows, sim_start):
        """Fail early when direct streamflow inputs need an explicit start date."""
        if predicted_streamflows is None or sim_start is not None:
            return
        for subbasin_id, streamflow_data in predicted_streamflows.items():
            if isinstance(streamflow_data, pd.Series):
                if isinstance(streamflow_data.index, pd.DatetimeIndex):
                    continue
            elif isinstance(streamflow_data, pd.DataFrame):
                if "time" in streamflow_data.columns:
                    continue
                if isinstance(streamflow_data.index, pd.DatetimeIndex):
                    continue
            raise ValueError(
                "predicted_streamflows were provided without sim_start, but "
                f"subbasin {subbasin_id} has no datetime index or 'time' column. "
                "Provide sim_start or include timestamps in every streamflow series."
            )
    ...
    def simulate_streamflow_at_outlets(
        ...
        override_area_normalize=None,
        override_log_transform=None,
        **scaling_options,
    ):
        ...
        override_area_normalize, override_log_transform : bool, optional
            Explicitly override model sidecar scaling settings. Leave as
            ``None`` to use the trained model configuration.
        ...
        Model scaling options are loaded from the trained model sidecar config
        when available. Use ``override_area_normalize`` or
        ``override_log_transform`` only when intentionally overriding that
        saved config.
        """
        if "area_normalize" in scaling_options:
            override_area_normalize = scaling_options.pop("area_normalize")
            warnings.warn(
                "area_normalize is deprecated in FloodMapper simulation calls; "
                "use override_area_normalize to make model-config overrides explicit.",
                DeprecationWarning,
            )
        if "log_transform" in scaling_options:
            override_log_transform = scaling_options.pop("log_transform")
            warnings.warn(
                "log_transform is deprecated in FloodMapper simulation calls; "
                "use override_log_transform to make model-config overrides explicit.",
                DeprecationWarning,
            )
        if scaling_options:
        ...
        vmodel = PredictStreamflow(self.working_dir)
        loaded_config = vmodel.load_model_config(model_path)
        if override_area_normalize is not None:
            if loaded_config and "area_normalize" in loaded_config and bool(loaded_config["area_normalize"]) != bool(override_area_normalize):
                warnings.warn(
                    "override_area_normalize differs from the trained model config. "
                    "Using the explicit override for flood-mapping simulation.",
                    RuntimeWarning,
                )
            vmodel.area_normalize = bool(override_area_normalize)
        if override_log_transform is not None:
            if loaded_config and "log_transform" in loaded_config and bool(loaded_config["log_transform"]) != bool(override_log_transform):
                warnings.warn(
                    "override_log_transform differs from the trained model config. "
                    "Using the explicit override for flood-mapping simulation.",
                    RuntimeWarning,
                )
            vmodel.log_transform = bool(override_log_transform)
    ...
    def extract_annual_peaks(self, streamflow_data, start_date=None):
        ...
        try:
            ...
            annual_peaks = series.resample("Y").max().dropna()
    ...
    def flood_frequency_analysis(self, annual_peaks, return_period):
        ...
        Notes
        -----
        Uses the Gumbel Extreme Value Type-I (EV1) distribution fitted by the
        Method of Moments (Chow 1964).  This is the conventional approach for
        annual-maximum flood frequency analysis (Bulletin 17C / WMO guidelines)
        and replaces the previous quadratic polynomial fit in log-return-period
        space, which could produce negative or non-monotonic estimates for short
        records (< ~10 years) and had no physical basis.
        ...
        For very short records (< 5 years) the estimate is unreliable; a warning
        is emitted.  For a return period beyond the sample, extrapolation is
        flagged in metadata but not blocked.
        """
        if return_period is None or float(return_period) <= 1:
            raise ValueError(f"return_period must be > 1 years. Received: {return_period}")

        peaks = annual_peaks["annual_peaks"].dropna().values.astype(np.float64)
        n = len(peaks)
        if n < 3:
            raise ValueError("At least three annual peaks are required for flood frequency analysis.")
        if n < 5:
            warnings.warn(
                f"Only {n} annual peaks available; Gumbel EV1 estimate is highly uncertain.",
                RuntimeWarning,
            )

        mu = float(np.mean(peaks))
        sigma = float(np.std(peaks, ddof=1))

        # Plotting positions for metadata (Gringorten, 1963)
        ranks = np.arange(1, n + 1)
        sorted_peaks = np.sort(peaks)
        exc_prob = (ranks - 0.44) / (n + 0.12)
        fitted_rp_min = float(1.0 / exc_prob[-1])   # smallest fitted RP (highest exceedance)
        fitted_rp_max = float(1.0 / exc_prob[0])     # largest fitted RP (lowest exceedance)

        # Gumbel frequency factor
        T = float(return_period)
        reduced_variate = -np.log(-np.log(1.0 - 1.0 / T))           # y_T
        # Method of Moments: relate reduced variate to standardised moments
        # mu_y = 0.5772 (Euler-Mascheroni), sigma_y = pi/sqrt(6)
        euler_mascheroni = 0.5772156649
        sigma_y = np.pi / np.sqrt(6.0)
        K_T = (reduced_variate - euler_mascheroni) / sigma_y          # frequency factor

        estimate = mu + K_T * sigma

        metadata = {
            "method": "Gumbel_EV1_MOM",
            "n_years": int(n),
            "sample_mean_m3s": mu,
            "sample_std_m3s": sigma,
            "frequency_factor_KT": float(K_T),
            "return_period_extrapolated": bool(
                T < fitted_rp_min or T > fitted_rp_max
            ),
            "fitted_return_period_min_years": fitted_rp_min,
            "fitted_return_period_max_years": fitted_rp_max,
        ...
        if estimate <= 0:
            warnings.warn(
                "Gumbel EV1 produced a non-positive discharge estimate "
                f"(QT={estimate:.3f} m3/s for T={T}-year). "
                "Using the maximum observed annual peak instead.",
                RuntimeWarning,
            )
            estimate = float(np.nanmax(peaks))
            metadata["negative_estimate_fallback"] = True

        return float(estimate), metadata
    ...
    def map_inundated_areas(
        ...
        **scaling_options,
        ...
        override_area_normalize, override_log_transform : bool, optional
            Explicitly override model sidecar scaling settings when simulating
            outlet hydrographs. Leave as ``None`` to use the trained model
            configuration.
        ...
        Model scaling options are loaded from the trained model sidecar config
        when ``model_path`` is used. Use ``override_area_normalize`` or
        ``override_log_transform`` only when intentionally overriding that
        saved config.
        """
        if "area_normalize" in scaling_options:
            override_area_normalize = scaling_options.pop("area_normalize")
            warnings.warn(
                "area_normalize is deprecated in map_inundated_areas; use "
                "override_area_normalize to make model-config overrides explicit.",
                DeprecationWarning,
            )
        if "log_transform" in scaling_options:
            override_log_transform = scaling_options.pop("log_transform")
            warnings.warn(
                "log_transform is deprecated in map_inundated_areas; use "
                "override_log_transform to make model-config overrides explicit.",
                DeprecationWarning,
            )
        if scaling_options:
            unknown = ", ".join(sorted(scaling_options))
        ...
        if not self.rating_curve_dict:
            self.load_rating_curves()
        self._validate_predicted_streamflow_dates(predicted_streamflows, sim_start)
        ...
        if predicted_streamflows is None:
            ...
            predicted_streamflows = self.simulate_streamflow_at_outlets(
                ...
                sim_start=sim_start,
                sim_end=sim_end,
                override_area_normalize=override_area_normalize,
                override_log_transform=override_log_transform,
                runoff_output_dir=runoff_output_dir,
            )
