...
class Utils:
    ...
    def require_existing_path(self, path, description, path_type="file"):
        ...
        exists = os.path.isdir(normalized_path) if path_type == "dir" else os.path.isfile(normalized_path)
        if not exists:
            noun = "directory" if path_type == "dir" else "file"
            raise FileNotFoundError(
                f"{description} {noun} was not found: {normalized_path}"
            )
        return normalized_path

    def require_columns(self, df, required_columns, source_name):
        """Ensure a tabular input contains the columns required by the API."""
    ...
    def require_study_area_gdf(self):
        """Load and validate the study-area shapefile."""
        self.require_existing_path(self.study_area, "Study area", path_type="file")
        try:
            gdf = gpd.read_file(self.study_area)
        except Exception as exc:
            raise ValueError(
                f"Study area could not be read as a vector dataset: {self.study_area}. "
                f"Original error: {exc}"
            ) from exc

        if gdf.empty:
            raise ValueError(f"Study area shapefile contains no features: {self.study_area}")
        if gdf.crs is None:
            raise ValueError(
                f"Study area shapefile has no CRS metadata: {self.study_area}"
            )
        return gdf

    def require_dem_reference(self):
        """Return the clipped DEM path if preprocessing has produced it."""
        dem_path = f'{self.working_dir}/elevation/dem_clipped.tif'
        return self.require_existing_path(
            dem_path,
            "Clipped DEM reference raster",
            path_type="file",
        )

    def validate_date_window(self, start_date, end_date, start_label="start_date", end_label="end_date"):
        """Validate YYYY-MM-DD input strings and chronological ordering."""
        try:
            start_dt = pd.to_datetime(start_date, format="%Y-%m-%d", errors="raise")
        except Exception as exc:
            raise ValueError(
                f"{start_label} must be in YYYY-MM-DD format. Received: {start_date}"
            ) from exc
        ...
        try:
            ...
            raise ValueError(
                f"{end_label} must be in YYYY-MM-DD format. Received: {end_date}"
            ) from exc

        if start_dt > end_dt:
            raise ValueError(
                f"{start_label} must be on or before {end_label}. "
                f"Received {start_dt.date()} > {end_dt.date()}."
            )
    ...
    def get_bbox(self, dst_crs):
        ...
        """
        shp = self.require_study_area_gdf()
        #dst_crs = 'EPSG:4326'
        dst_crs = dst_crs
    ...
    def clip(self, raster_path,  dst_crs='EPSG:4326', out_path=None, save_output=False, crop_type=False):
        ...
        """
        self.require_existing_path(raster_path, "Raster to clip", path_type="file")
        shp = self.require_study_area_gdf()
        #dst_crs = 'EPSG:4326'
        dst_crs = dst_crs
        if shp.crs.equals(dst_crs):
